...
func collision_received(sender: String, sender_position: Vector2) -> void:
	if !tween.is_active() and (SPIDER_WEB_NAME in sender):
		hunt(get_point_path_to(sender_position))
